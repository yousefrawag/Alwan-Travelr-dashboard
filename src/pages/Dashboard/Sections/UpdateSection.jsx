...
const UpdateSection = () => {
    const { id } = useParams();
    const { data, isLoading:getloading } = useQuerygetSpacficIteam("Section", "Section", id);
  const {isLoading , updateiteam} = useQueryupdate("Section" , "Section")
      ...
      try {
        ...
        updateiteam({data , id} , {
              onSuccess:() =>{
                  ...
                  e.target.reset()
                  toast.success("تم تعديل  قسم بنجاح")
                  navigate("/Sections")
              },  
               onError: (error) => {
